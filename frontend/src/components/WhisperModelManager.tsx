    ...
    const setupListeners = async () => {
      ...
      unlistenComplete = await listen<{ modelName: string }>(
        ...
        (event) => {
          ...
          toast.success(`${getModelIcon(model?.accuracy || 'Good')} ${displayName} 已就绪！`, {
            description: '模型已下载，可以使用',
            duration: 4000
          });
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      {advancedModels.length > 0 && (
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="advanced-models">
            <AccordionTrigger>
              <span className='text-lg'>高级模型</span>
            </AccordionTrigger>
            <AccordionContent>
      ...
      {selectedModel && (
        ...
        >
          正在使用 {getDisplayName(selectedModel)} 进行转录
        </motion.div>
      )}
    </div>
  );
        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-600 ml-9 mt-1.5">
              ...
              <span className="flex items-center space-x-1">
                <span>🎯</span>
                <span>{accuracyLabel(model.accuracy)} 精度</span>
              </span>
              <span className="flex items-center space-x-1">
                <span>⚡</span>
                <span>{speedLabel(model.speed)} 处理速度</span>
              </span>
            </div>
          </div>
          ...
          <div className="ml-4 flex items-center gap-2">
            ...
            {isMissing && (
              <button
                ...
                className="bg-blue-600 text-white px-3 py-1.5 rounded-md text-sm font-medium hover:bg-blue-700 transition-colors shadow-sm hover:shadow"
              >
                下载
              </button>
            )}
            ...
            {downloadProgress === null && isError && (
              <button
                ...
                className="bg-red-600 text-white px-3 py-1.5 rounded-md text-sm font-medium hover:bg-red-700 transition-colors shadow-sm hover:shadow"
              >
                重试
              </button>
            )}
            ...
            {isCorrupted && (
              <div className="flex gap-2">
                <button
                  ...
                  className="bg-orange-600 text-white px-3 py-1.5 rounded-md text-sm font-medium hover:bg-orange-700 transition-colors shadow-sm hover:shadow"
                >
                  删除
                </button>
                <button
